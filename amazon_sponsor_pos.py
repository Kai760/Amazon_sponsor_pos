...
from utils.template_matching import non_max_supression, delete_not_aimed_sponsor, get_template_position
...
def print_result(sponsor_rank):
    if sponsor_rank == -1:
        print("target画像が見つかりませんでした。")
    elif sponsor_rank == 0:
        print("target画像はありましたが、スポンサープロダクトではありませんでした。")
    else:
        print(f"ターゲットは{sponsor_rank}番目のスポンサープロダクトです。")
...
def main(whole_img_path, target_img_path, ini_path, sponsor_img_path='sponsor/sponsor.png'):
    ...
    print("ターゲット画像位置取得")
    loc_target, target_w, target_h = get_template_position(images.whole_img, images.target_img, 0.95, 0.03)
    ...
    print("\nスポンサープロダクト位置取得")
    # 閾値0.7 &許容範囲0.15でいいのか？？
    loc_sponsor, sponsor_w, sponsor_h = get_template_position(images.whole_img, images.sponsor_img, 0.7, 0.15)
    ...
    distance = 10000
    sponsor_rank = 100
    for target_focus in zip(*loc_target[::-1]):
        target_focus_w, target_focus_h = target_focus
        cv2.rectangle(images.img_rgb, target_focus, (target_focus_w + target_w, target_focus_h + target_h), (0, 0, 255), 2)
        for index, pt in enumerate(zip(*loc_sponsor[::-1])):
            cv2.rectangle(images.img_rgb, pt, (pt[0] + sponsor_w, pt[1] + sponsor_h), (0, 0, 255), 2)
            temp_distance = abs(pt[0] - target_focus_w) + abs(pt[1] - (target_focus_h + target_h))
            if temp_distance < distance:
                distance = temp_distance
                sponsor_rank = index + 1
    # スポンサーに対する矩形かどうかを距離から確認
    sponsor_rank = sponsor_rank if images.whole_w * 0.1 > distance else 0

    # 今回の拡縮情報をiniファイルに書き込み
    images.write_ini(images.whole_w, target_w, sponsor_w)

    # 結果確認(デバッグ用)
    cv2.imwrite(f'result_no{sponsor_rank}.png', images.img_rgb)
    return sponsor_rank
...
def debug(logger):
    ...
    for product in products:
        ...
        sponsor_rank = main(whole_img, target_img, ini_path)

        logger.debug(f"{basename}:pos={sponsor_rank}, {time.time() - start}s")
        times.append(time.time() - start)
        print_result(sponsor_rank)
...
if __name__ == '__main__':
    ...
    # スポンサー順位を取得
    sponsor_rank = main(opt.whole_img, opt.target_img, opt.ini_path)
    logger.debug(f"{os.path.basename(opt.target_img)}:pos={sponsor_rank}")
    print_result(sponsor_rank)
